<template>
  <n-card title="Domain Options" size="small">
    ...
    <n-tabs type="segment">
      <n-tab-pane name="allow-list" tab="Allowed">
        <div>
          <n-list bordered>
            <template #header>
              These sites will <strong>not</strong> be censored, regardless of
              mode!<br />
              A site only needs to <em>partially</em> match any of the values
              below to be allowed.
            </template>
            <n-list-item v-for="match in allowed" v-bind:key="match">
              {{ match }}
              <template #suffix>
                <n-button @click="allowed.splice(allowed.indexOf(match), 1)"
                  >Remove</n-button
                >
              </template>
            </n-list-item>
      ...
      <n-tab-pane name="force-list" tab="Forced">
        <div>
          <n-list bordered>
            <template #header>
              These sites will be automatically censored, even in On Demand
              mode.<br />
              A site only needs to <em>partially</em> match any of the values
              below to be censored.
            </template>
            <n-list-item v-for="match in forced" v-bind:key="match">
              {{ match }}
              <template #suffix>
                <n-button @click="forced.splice(forced.indexOf(match))"
                  >Remove</n-button
                >
              </template>
            </n-list-item>
    ...
    <template #footer
      >The Allowed list overrides the Forced list in On Demand mode. Your
      changes are automatically saved.</template
    >
  </n-card>
</template>
...
import {
  updateUserPrefs,
  userPrefs,
} from "@silveredgold/beta-shared-components";
